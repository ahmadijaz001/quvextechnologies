...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://akross.ae/services/it-peripherals/${serviceSlug}`,
    },
    openGraph: {
      title: `${service.name} | aKross UAE`,
      description: service.overview.slice(0, 160),
      url: `https://akross.ae/services/it-peripherals/${serviceSlug}`,
      type: "website",
      locale: "en_AE",
    },
  };
...
export default async function ITPeripheralsServiceDetailPage({ params }: Props) {
  const { serviceSlug } = await params;
  const category = serviceCategories.find(c => c.slug === "it-peripherals")!;
  const service = category.services.find(s => s.slug === serviceSlug);
  if (!service) notFound();
